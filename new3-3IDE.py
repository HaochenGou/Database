...
class Interface:
	...
	def login(self):
		while self.app_continue and self.valid_login == False:
			...
			if re.match("^[A-Za-z0-9_]*$", username) and re.match("^[A-Za-z0-9_]*$", password):
				...
				data = (password, )
				user_pwd = self.c.execute('SELECT pwd FROM users WHERE uid=?;' , (username,)).fetchall()
				if password!= user_pwd[0][0]:
					print('Invalid uid or password.')
				else:
					self.valid_login = True
				self.utype = self.c.execute('select utype from users where uid=?;',(username,)).fetchall()
	...
	def regbirth(self):
		#while True:
			#try:
				...
				self.c.execute('SELECT * FROM persons WHERE fname = ? and lname  = ?;',( m_f_name,  m_l_name))
				mother_info = self.c.fetchall()
				if len(mother_info) > 0:
					print("Found mother's info in system.")
					address = mother_info[0][4]
					phone = mother_info[0][5]
				else:
					self.addPerson(m_f_name, m_l_name)
				
				
				#father
				f_f_name = input("Father's first name: ")
				f_l_name = input("Father's last name: ")
				self.c.execute('SELECT * FROM persons WHERE fname = ? and lname =?;',(f_f_name, f_l_name))
				father_info = self.c.fetchall()
				if len(father_info) > 0:
					print("Found fathers's info in system.")
				else:
					self.addPerson(f_f_name, f_l_name)
	...
	def getRegno(self):
		...
		while True:
			...
			self.c.execute('SELECT * FROM births WHERE regno =:num',{"num":ran_num})
			val = self.c.fetchone()
			if bool(val) == False:
				return(ran_num)
	...
	def addPerson(self,f_name,l_name):
		...
		if telnum == '':
			telnum = None
		self.c.execute('INSERT INTO persons VALUES (:fname,:lname,:bdate,:bplace,:add,:phone)',
	...
	def getMarrNo(self):
		...
		while True:
			ran_num = random.randint(0,1000000)
			self.c.execute('SELECT * FROM marriages WHERE regno =:num',{"num":ran_num})
			val = self.c.fetchone()
			if bool(val) == False:
				return(ran_num)
	...
	def renew_vehreg(self):
		#while True:
			#try:
				...
				self.c.execute('SELECT * FROM registrations where regno= :regno', {'regno':regno})
				
				old_reg = self.c.fetchone()
				...
				#update registration
				self.c.execute('UPDATE registrations SET expiry=:new_exp WHERE regno=:reg',
	...
	def process_sale(self):
		...
		get_data = self.c.execute('select * from registrations where vin =:vin limit 1', {"vin": car_vin}).fetchall()
		self.conn.commit() 
		while  cur_fn.lower() != get_data[0][5].lower():
			print ('Wrong name.')
			cur_fn = input("Please input first name of current onwer: ")
		while  cur_ln.lower() != get_data[0][6].lower():
		...
		regno_data = 0
		regno_data = self.c.execute('select * from registrations where regno =:regno limit 1', {"regno": new_regno}).fetchall()
		new_regno= random.randint(100, 9999) # get new regno
		regno_data = self.c.execute('select * from registrations where regno =:regno limit 1', {"regno": new_regno}).fetchall()    
		old_data = (exp_date, get_data[0][4])
		self.c.execute('update registrations set expiry = ? where vin = ?;', old_data)
		self.conn.commit() 
		new_data = (new_regno, exp_date, new_date, get_data[0][3], get_data[0][4], new_fn, new_ln)
		self.c.execute('insert into registrations values(?,?,?,?,?,?,?);',new_data)
		self.conn.commit() 
		print("Sale done.")
		choice = 0
		while choice != "r":
	...
	def payment(self):
		...
		while get_data == 0:
			...
			get_data = self.c.execute('select * from tickets where tno =:tno limit 1', {"tno": t_tno}).fetchall()  
			self.conn.commit() 
		if get_data == 0 :
			print("invaild ticket number.")
		t_payment = 0
		while t_payment == 0:
			t_payment = float(input("Please input payment: "))
			if  t_payment  == 0:
				print('Invalid payment.')       
          
		while float(t_payment) > float(get_data[0][2]):
			print('Invalid payment.')
			t_payment = input("Please input payment: ")
		pdate = datetime.date(datetime.now())	
		payments = (t_tno, pdate,  t_payment)
		self.c.execute('insert into payments values(?,?,?);',payments)
		get_pay = self.c.execute('select * from payments where tno =:tno', {"tno": t_tno}).fetchall()
		sum_pay = float(get_pay[0][2]) +  float(t_payment)
		while sum_pay > float(get_data[0][2]):
			print('Invalid payment.')
			t_payment = input("Please input payment: ")
			sum_pay = float(get_pay[0][2]) +  float(t_payment)
	...
	def getAbstract(self):
		...
		get_data = self.c.execute(countTicket_query,name).fetchall()
		count_ticket =int(get_data[0][0])    
    
		countNotice_query  =  "select count(*) from demeritNotices where fname = ? and lname = ?;" 
		get_data = self.c.execute(countNotice_query,name).fetchall()
		count_Notice = int(get_data[0][0])   
		
		countPoints_query = "select sum(points) from demeritNotices where fname = ? and lname = ?;" 
		get_data = self.c.execute(countPoints_query,name).fetchall()
		count_points = int(get_data[0][0])     
		...
		get_data = self.c.execute(count2Ticket_query,name).fetchall()
		count2_ticket = int(get_data[0][0])     
    
		count2Notice_query = "select count(*) from demeritNotices where fname = ? and lname = ? and ddate >= DATE('NOW', '-2 YEAR');"
		get_data = self.c.execute(count2Notice_query,name).fetchall()
		count2_Notice = int(get_data[0][0])    
    
		count2Points_query = "select sum(points) from demeritNotices where fname = ? and lname = ? and ddate >= DATE('NOW', '-2 YEAR');"
		get_data = self.c.execute(count2Points_query,name).fetchall()
		count2_points = int(get_data[0][0])     
		self.conn.commit()
		...
		if choice.lower() == 'y':
			...
			get_data = self.c.execute(display_query,name).fetchall()
			print(get_data)        
	...
	def Check_info(self, regis_num):
		self.c.execute('''Select b.fname||b.lname, v.make, v.model, v.year, v.color
	...
	def random_num(self):
		while True:
			...
			self.c.execute('Select * from tickets t Where t.tno = ?', (ticket_num,))
			check_num = self.c.fetchall()
			if (len(check_num)) != 0:
	...
	def Issue_ticket(self, regis_num):
		...
		ticket_num = self.random_num()
		ticket_info = (ticket_num, regis_num, v_fine, v_violation, v_date)
		self.c.execute('''Insert into tickets Values(?,?,?,?,?)''', (ticket_info))
		self.conn.commit()
	...
	def Question8(self):
		while True:
			try:
				...
				print('Registration number is not integer!')
			self.c.execute("select * from registrations where regno = ?",(regis_num,))
			finding = self.c.fetchall()
			if len(finding) == 0:
