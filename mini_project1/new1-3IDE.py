...
class Interface:
	...
	def login(self):
		while self.app_continue and self.valid_login == False:
			...
			if re.match("^[A-Za-z0-9_]*$", username) and re.match("^[A-Za-z0-9_]*$", password):
				...
				#re.create_function("hash", 1, self.encrypt)
				data = (username, password)
				#self.c.execute(" INSERT INTO users (uid, pwd) VALUES (?, hash(?)) ", data )
				data = (password, )
				#uid = self.c.execute(" SELECT uid FROM member WHERE pwd LIKE hash(?) ", data).fetchall()
				self.c.execute('SELECT * FROM users WHERE pwd=?;' , (password,))
				user_uid = self.c.fetchall()
				if username != user_uid[0][0]:
	...
	def regbirth(self):
		...
		while valid_entry == False:
			try:
				...
				self.c.execute('SELECT * FROM persons WHERE fname like :mfname and lname like :mlname',
				...
				self.c.execute('SELECT * FROM persons WHERE fname like :ffname and lname like :flname',
	...
	def process_sale():
		...
		while regno_data != NUll:
			new_regno= random.randint(100, 9999) # get new regno
			regno_data = c.execute('select * from registrations where regno =:regno limit 1', {"regno": new_regno}).fetchall()        
		old_data = (exp_date, get_data[0][4])
		self.c.execute('update registrations set expiry = ? where vin = ?;', old_date)
		self.conn.commit() 
